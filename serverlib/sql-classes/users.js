...
export default class UsersSQL {
    ...
    static async getByUsername(username) {
        const data = await psqlQuery("SELECT * FROM users WHERE username=$1", [username])

        return data[0]
    }

    static async getByUsernameAndPassword(username, password) {
        const data = await psqlQuery(`SELECT * FROM users WHERE "username"=$1 AND "password"=(SUBSTRING("password"::TEXT FROM 0 FOR 65) || encode(sha256(SUBSTRING("password"::BYTEA FROM 0 FOR 65) || $2), 'hex'));`, [username, password])
