...
const psqlQuery = (query, params) => {
    if (params != null && typeof params != "object") params = [params]
...
export const psqlInsert = async (table, values) => {
    let valuesFillin = []
    ...
    let query = `INSERT INTO ${table} (${Object.keys(values).join(", ").toLowerCase()}) VALUES (${valuesFillin.join(", ").toLowerCase()})`

    await psqlQuery(query, valuesValues)
}

export const psqlUpdate = async (table, values, where) => {
    let finalValues = []
    let updateValues = []
    let whereValues = []
    ...
    let query = `UPDATE ${table} SET ${updateValues.join(", ")} WHERE ${whereValues.join(", ")}`
